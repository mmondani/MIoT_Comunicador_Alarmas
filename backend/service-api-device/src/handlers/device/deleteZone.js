...
import deleteZoneSchema from '../../schemas/deleteZoneSchema';
...
async function deleteZone(event, context) {
    ...
    try {
        let response = await db.collection("devices")
            .updateOne(
                {comId: comId},
                {$pull:{"particiones.$[p].zonas":{numero: numero}}},
                {arrayFilters: [{"p.numero": particion}]}
            );

        if (response.modifiedCount === 0)
            return httpStatus(409, {error: `Error al eliminar la zona ${numero} del comunicador ${comId}`})
    }
    catch (error) {
...
export const handler = commonMiddleware(deleteZone)
    .use(validator({
