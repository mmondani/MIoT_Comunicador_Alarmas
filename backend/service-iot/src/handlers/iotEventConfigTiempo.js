...
import { parseHeader, parseRegisterConfigTiempo } from "../lib/parser";
...
async function iotEventConfigTiempo(event, context) {
  ...
  let payloadParsed = parseRegisterConfigTiempo(parsedMessage);
  ...
  try {
    ...
    await db.collection("devices").updateOne (
      {comId: comId},
      {$set:{
        sincronizaHora: payloadParsed.sincronizaHora,
        codigoRegion: payloadParsed.codigoRegion
      }}
    );
...
export const handler = iotEventConfigTiempo;
